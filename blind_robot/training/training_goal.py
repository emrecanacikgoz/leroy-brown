...
def process_batch(model, goal_encoder, opt, loader, device, train=False):

    loss_eef_pos_value = []
    loss_eef_rot_value = []
    loss_gripper_value = []
    for batch in loader:
        inputs, targets, _, end_states, mask = batch
        ...
        # goal encoder
        embed_g = goal_encoder(inputs, end_states)

        # policy
        actions_pred = model(inputs, embed_g)
...
def train_batch(model, goal_encoder, opt, train_loader, device):
    model.train()
    goal_encoder.train()
    train_loss = process_batch(model, goal_encoder, opt, train_loader, device, train=True)
    return train_loss


def valid_batch(model, goal_encoder, opt, valid_loader, device):
    model.eval()
    goal_encoder.eval()
    with torch.no_grad():
        valid_loss = process_batch(model, goal_encoder, opt, valid_loader, device, train=False)
