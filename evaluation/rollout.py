...
class AddGaussianNoise(object):
    ...
    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        return tensor + torch.randn(tensor.size()).to("cuda:0") * self.std + self.mean
...
def rollout(
    ...
    add_noise = AddGaussianNoise(mean=0.0, std=0.01)
    ...
    robot_obs = episode["state_info"]["robot_obs"][None, 0:1, :]
    scene_obs = episode["state_info"]["scene_obs"][None, 0:1, :]
    

    robot_obs_init = robot_obs 
    scene_obs_init = scene_obs
    start_state = torch.tensor(get_features(scene_obs_init, robot_obs_init, input_features), device=device).float()
    ...
    for step in range(84):
        ...
        if add_noise_during_inference:
            inputs, end_state = add_noise(inputs), add_noise(end_state)

        lang_embeddings= language_encoder(inputs, start_state, task_id)
        #goal_embeddings = goal_encoder(inputs, end_state)

        action = transformer.step(inputs, lang_embeddings)
